...
public class CheckboxTag extends ElementsTag {
    ...
    protected void __doSetTagName() {
        if (this.getParent() instanceof FormControlTag) {
            __formControl = (FormControlTag) this.getParent();
            this.set_tag("input");
            this.set_unclosed(true);
            this.getDynamicAttributes().put("type", __formControl.getType());
        }
    }
    ...
    protected StringBuilder __doTagContent(StringBuilder tagContent, StringBuilder bodyContent) {
        if (this.getParent() instanceof FormControlTag) {
            ...
            _tmpSB.append("<label");
            if (__formControl != null && __formControl.isInline()) {
                _tmpSB.append(" class=\"").append(__formControl.getType()).append("-inline\"");
            }
            _tmpSB.append(">");
            tagContent.insert(0, _tmpSB).append(bodyContent).append("</label>");
        }
        return tagContent;
    }
